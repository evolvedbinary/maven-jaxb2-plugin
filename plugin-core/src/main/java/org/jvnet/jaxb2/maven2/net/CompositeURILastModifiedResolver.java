...
public class CompositeURILastModifiedResolver implements URILastModifiedResolver
{
  ...
  public CompositeURILastModifiedResolver (final Log logger)
  {
  ...
  private void addResolvers (final SchemeAwareURILastModifiedResolver... resolvers)
  {
  ...
  private URILastModifiedResolver getResolver (final String scheme)
  {
    return this.resolvers.get (scheme);
  }

  @Override
  public Long getLastModified (final URI uri)
  {
    final String scheme = uri.getScheme ();
    if (scheme == null)
    {
      getLogger ().error (MessageFormat.format ("URI [{0}] does not provide the scheme part.", uri));
    }
    final URILastModifiedResolver resolver = getResolver (scheme);
    if (resolver == null)
    {

      getLogger ().error (MessageFormat.format ("Could not resolve the last modification of the URI [{0}] with the scheme [{1}].",
                                                uri,
                                                scheme));
    }
    else
